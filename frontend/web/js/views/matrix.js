...
export async function render(root) {
  ...
  const rows = dets.map(n => `<tr>
      ...
      ${datasets.map(d => {
        ...
        return `<td class="cell" title="${esc(shortName(n))} · ${esc(dsLabel(d))} = F1 ${dec(c.f1)} (rank ${roman(c.rank)})"
  ...
  el.innerHTML = `<div class="panel" style="overflow:auto">
      <table class="heat mtx">${head}${rows}</table>
    </div>
    <p class="muted-note mt">darker cell = higher F1 · <span class="num">I / II / III</span> = rank within dataset · leader changes per dataset = no universally best detector.</p>`;
}
